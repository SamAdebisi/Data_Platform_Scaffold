...
def main():
    ...
    try:
        while True:
            msg=c.poll(1.0)
            if msg is None:
                if batch: write_parquet_s3(batch); c.commit(); batch=[]
                continue
            if msg.error(): raise KafkaException(msg.error())
            rec=json.loads(msg.value())
            if not validate(rec, SCHEMA): continue
            batch.append(rec)
            if len(batch)>=500:
                write_parquet_s3(batch); c.commit(); batch=[]
    finally:
        if batch: write_parquet_s3(batch); c.commit()
        c.close()
